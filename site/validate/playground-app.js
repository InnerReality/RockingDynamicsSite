...
(() => {
  ...
  function refreshReadout(input) {
    ...
    if (!val) return;
    const dec = input.step.indexOf(".") >= 0 ? input.step.split(".")[1].length : 0;
  ...
  function buildExcitationFn(axis) {
    ...
    if (preset === "sine") {
      ...
      return (t) => (t >= tstart ? amp * Math.sin(2 * Math.PI * freq * (t - tstart) + phase) : 0);
    }
    if (preset === "pulse") {
  ...
  function updatePresetVisibility(axis) {
    const preset = document.getElementById(`pg-${axis}-preset`).value;
    document.querySelectorAll(`.pg-preset-params[data-for="${axis}"] .p`).forEach((el) => {
      // each param block carries the preset name(s) it belongs to in its
      // class; "common" blocks (amplitude, start time) are always shown
      const show = el.classList.contains("common") || el.classList.contains(preset);
  ...
  function setAyLocked(locked) {
    ...
    document.querySelectorAll('.pg-preset-params[data-for="ay"] input').forEach((el) => {
      el.disabled = locked;
    });
  }
  ...
  function buildPlot(spec) {
    ...
    const Hh = spec.height ?? 270;
    const hasLegend = spec.series.some((ser) => ser.label);
    const L = 72, R = 12, T = hasLegend ? 56 : 38, B = hasLegend ? 50 : 44;
    const xs = spec.series.flatMap((s2) => s2.pts.map((p) => p[0]));
    const ys = spec.series.flatMap((s2) => s2.pts.map((p) => p[1]));
    let x0 = Math.min(...xs), x1 = Math.max(...xs);
    let y0 = Math.min(...ys), y1 = Math.max(...ys);
  ...
  function buildInputPlots(u_, suffix, track) {
    ...
    const axPlot = buildPlot({
      ...
      series: [{ pts: ds(u_.map((r, i) => [i * 1e-3, r[0] / 386.4])), color: "#0072BD" }]
    });
    const ayPlot = buildPlot({
      ...
      series: [{ pts: ds(u_.map((r, i) => [i * 1e-3, r[1] / 386.4])), color: "#D95319" }]
    });
    const orbitPlot = buildPlot({
      ...
      width: w, height: oh,
      equal: true,
      markers: track ? [{ id: `mk-orbit${suffix}`, color: "#dc2626" }] : undefined,
      series: [{ pts: ds(u_.map((r) => [r[0] / 386.4, r[1] / 386.4])), color: "#0072BD" }]
    });
  ...
  function buildDrawerPreview(u_) {
    const axayPlot = buildPlot({
      ...
      series: [
        { pts: ds(u_.map((r, i) => [i * 1e-3, r[0] / 386.4])), color: "#0072BD", label: "aₓ" },
        { pts: ds(u_.map((r, i) => [i * 1e-3, r[1] / 386.4])), color: "#D95319", label: "aᵧ" }
      ]
    });
    const orbitPlot = buildPlot({
      title: "Excitation orbit aᵧ vs aₓ",
  ...
  function buildForcePlots() {
    ...
    const serA = { pts: ds(T.map((t, i) => [t, res.fBWtotal[i][iA]])), color: "#0072BD", label: `stack ${iA + 1}` };
    const serB = iB !== null ? { pts: ds(T.map((t, i) => [t, res.fBWtotal[i][iB]])), color: "#D95319", label: `stack ${iB + 1}` } : null;
    const cursors = [{ id: "cur-force-a", seriesIndex: 0, color: "#0072BD", label: String(iA + 1) }];
    if (serB) cursors.push({ id: "cur-force-b", seriesIndex: 1, color: "#D95319", label: String(iB + 1) });
    ...
    const hystA = { pts: ds(res.Delt.map((r, i) => [r[iA], res.fBWtotal[i][iA]])), color: "#0072BD", label: `stack ${iA + 1}` };
    const hystB = iB !== null ? { pts: ds(res.Delt.map((r, i) => [r[iB], res.fBWtotal[i][iB]])), color: "#D95319", label: `stack ${iB + 1}` } : null;
    const markers = [{ id: "mk-hyst-a", color: "#0072BD", label: String(iA + 1) }];
    if (hystB) markers.push({ id: "mk-hyst-b", color: "#D95319", label: String(iB + 1) });
  ...
  function buildPlots() {
    ...
    const thetaPlot = buildPlot({
      ...
      series: [{ pts: ds(T.map((t, i) => [t, theta[i] * DEG])), color: "#0072BD" }]
    });
    const phiPlot = buildPlot({
      ...
      series: [{ pts: ds(T.map((t, i) => [t, res.Phi[i] * DEG])), color: "#0072BD" }]
    });
  ...
  function showStaticBushing() {
    ...
    staticAnimator = createBushingAnimator(canvas, { t: [0], phi: [0], theta: [0] }, {
      geom: { h1: 24, d1: 12, h2: 1, d2: 2 * numVal("pg-r0"), h3: 72, d3: 16, r0: numVal("pg-r0") },
      ssign,
      thetaGain: 50,
      stackRadius: 10.5,
      nStacks: 12,
  ...
  function runSimulation() {
    ...
    worker.onmessage = (e) => {
      ...
      if (staticAnimator) { staticAnimator.destroy(); staticAnimator = null; }
      ...
      const defaultGain = maxAbs > 1e-9 ? Math.max(3, Math.min(100, Math.round(0.3 / maxAbs))) : 50;
      ...
      animOpts = {
        ...
        onUpdate: (frame) => {
          ...
          plotHandles.orbitPlot.setMarker("mk-orbit", res.u_[ui][0] / 386.4, res.u_[ui][1] / 386.4);
          ...
          plotHandles.hystPlot.setMarker("mk-hyst-a", res.Delt[fi][selStacks[0]], res.fBWtotal[fi][selStacks[0]]);
          if (selStacks[1] !== null) {
            plotHandles.hystPlot.setMarker("mk-hyst-b", res.Delt[fi][selStacks[1]], res.fBWtotal[fi][selStacks[1]]);
          }
      ...
      let maxTheta = 0, maxThetaT = 0;
      for (let i = 0; i < theta.length; i++) {
        const a = Math.abs(theta[i]);
        if (a > maxTheta) { maxTheta = a; maxThetaT = T[i]; }
      }
      let maxAx = 0;
  ...
  function activateTab(btnId) {
    document.querySelectorAll("#pg-tabs .tab").forEach((b) => b.classList.remove("active"));
    document.querySelectorAll(".sidebar.right .panelrow").forEach((p) => p.classList.remove("active"));
  ...
  ["ax", "ay"].forEach((axis) => {
    document.getElementById(`pg-${axis}-preset`).addEventListener("change", () => {
      updatePresetVisibility(axis);
      if (axis === "ax" && ayLinked()) {
        document.getElementById("pg-ay-preset").value = document.getElementById("pg-ax-preset").value;
        updatePresetVisibility("ay");
      }
      activateTab("tab-exc");
      markDirty();
  ...
  for (const [btnId, panelId] of [
    ...
    document.getElementById(btnId).addEventListener("click", () => activateTab(btnId));
  }
