(() => {
  ...
  const downloadLabel =
    '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 1a.75.75 0 0 1 .75.75v7.69l2.22-2.22a.75.75 0 1 1 1.06 1.06l-3.5 3.5a.75.75 0 0 1-1.06 0l-3.5-3.5a.75.75 0 1 1 1.06-1.06l2.22 2.22V1.75A.75.75 0 0 1 8 1zM2.75 13.5h10.5a.75.75 0 0 1 0 1.5H2.75a.75.75 0 0 1 0-1.5z"/></svg><span>GIF</span>';
  ...
  const runButton = document.querySelector(
    canvas.id === "ma-canvas" ? "#ma-run" : "#pg-run",
  );
  const syncRunState = () => {
  ...
  if (runButton) {
    new MutationObserver(syncRunState).observe(runButton, {
      attributes: true,
  ...
  function sampleFrame(source, width, height) {
    ...
    for (let i = 0, px = 0; i < raw.length; i += 4, px++) {
      ...
      if (best === undefined) {
        ...
        const isGray =
          Math.max(raw[i], raw[i + 1], raw[i + 2]) -
            Math.min(raw[i], raw[i + 1], raw[i + 2]) <=
          8;
        for (let p = 0; p < palette.length; p++) {
          if (
            isGray &&
            !(
              palette[p][0] === palette[p][1] && palette[p][1] === palette[p][2]
            )
          )
            continue;
          const d =
            (raw[i] - palette[p][0]) ** 2 +
            (raw[i + 1] - palette[p][1]) ** 2 +
            (raw[i + 2] - palette[p][2]) ** 2;
          if (d < distance) {
            distance = d;
            best = p;
          }
  ...
  button.addEventListener("click", async () => {
    ...
    const slider = document.querySelector(
      isMinAccel ? "#ma-slider" : "#pg-slider",
    );
    const playButton = document.querySelector(
      isMinAccel ? "#ma-play" : "#pg-play",
    );
    ...
    const simulatorFramesPerGifFrame =
      (lastFrame * playbackSpeed) / (duration * framesPerGifSecond);
    const frameNumbers = [];
    for (
      let n = 0, frame = 0;
      frame <= lastFrame;
      n++, frame = Math.round(n * simulatorFramesPerGifFrame)
    ) {
      if (frameNumbers[frameNumbers.length - 1] !== frame)
        frameNumbers.push(frame);
    }
    if (frameNumbers[frameNumbers.length - 1] !== lastFrame)
    ...
    if (wasPlaying && playButton?.textContent.trim() === "Play")
      playButton.click();
